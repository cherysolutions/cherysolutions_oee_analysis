...
with s   as (select * from {{ ref('stg_production_events') }}),
...
from s
join dm  on dm.machine_code   = s.machine_code
join ds  on ds.sku_code       = s.sku_code
join dsh on dsh.shift_code    = s.shift_code
join dd  on dd.date_key       = s.date_key
